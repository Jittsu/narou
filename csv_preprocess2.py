...
def main():
    ...
    df2.review_cnt = pd.DataFrame(mms.fit_transform((df2.review_cnt.values).reshape(-1,1))) #love2
    df2.length = pd.DataFrame(mms.fit_transform((df2.length.values).reshape(-1,1))) #love2
    df2.time = pd.DataFrame(mms.fit_transform((df2.time.values).reshape(-1,1))) #love2
    # log変換=>love3
    ...
    df1.review_cnt = df1.review_cnt.apply(lambda x: np.log(x+1))
    df1.length = df1.length.apply(lambda x: np.log(x+1))
    df1.time = df1.time.apply(lambda x: np.log(x+1))
    df1.general_all_no = df1.general_all_no.apply(lambda x: np.log(x+1))
    #df1.time_info = df.time_info.apply(lambda x: np.log(x+1))
    df1.dayup_ave = df1.dayup_ave.apply(lambda x: np.log(x+1))
